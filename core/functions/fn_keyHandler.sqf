...
switch (_code) do {
        ...
        if (!_alt && !_ctrlKey && !dialog && {!life_action_inUse}) then {
            if (!(isNull objectParent player) && alive vehicle player) then {
                ...
                if (!(isNil "_list")) then {
                    ...
                    if (KINDOF_ARRAY(cursorObject,_list) && {player distance cursorObject < 12} && {isNull objectParent player} && {alive cursorObject} && {!life_action_inUse}) then {
    ...
    case 38: {
        ...
        if (_shift && playerSide in [west,independent]) then {
            if (!(isNull objectParent player) && (typeOf vehicle player) in ["C_Offroad_01_F","C_SUV_01_F","C_Van_01_box_F","O_Truck_02_Ammo_F","B_MRAP_01_F","C_Hatchback_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_03_unarmed_F","O_Heli_Transport_04_medevac_F","C_Heli_Light_01_civil_F","I_Heli_Light_03_unarmed_F","B_Heli_Transport_01_F"]) then {
   ...
   // Ä Schranken öffnen
	...
	case 37:
    {
        if (!_shift && !_alt && !_ctrlKey && (playerSide == west) && (vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F", "C_SUV_01_F","C_Hatchback_01_sport_F", "C_Hatchback_01_F", "B_Truck_01_transport_F", "B_MRAP_01_F", "O_MRAP_02_F", "I_MRAP_03_F", "B_MRAP_01_hmg_F", "O_MRAP_02_hmg_F","C_Heli_Light_01_civil_F","B_Heli_Transport_01_F","I_Heli_light_03_unarmed_F","B_Heli_Light_01_F"])) then
        {
	...
	case 62:
	{
		if(_alt && !_shift) then 
		{
			...
			diag_log format ["Servernachricht: %1 hat ALT+F4 benutzt um sich dem Rollenspiel zu entziehen (Mach einen Screenshot von dieser Meldung und poste diesen im Forum!)",_player getVariable["realname",name _player]];
			[[1,format["Servernachricht: %1 hat ALT+F4 benutzt um sich dem Rollenspiel zu entziehen (Mach einen Screenshot von dieser Meldung und poste diesen im Forum!)",_player getVariable["realname",name _player]]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
			SOCK_fnc_updateRequest;
		};
	};
};
